...
{
    simple_god_message("(은)는 당신의 몸을 변형했다.");
...
bool do_god_gift(bool forced)
{
    ...
    {
        mprf(MSGCH_DIAGNOSTICS, "총 신으로부터 받은 선물 수 : %d",
             you.num_total_gifts[you.religion]);
    }
#endif
    return success;
...
void excommunication(bool voluntary, god_type new_god)
{
    ...
    case GOD_GOZAG:
        ...
        {
            mprf(MSGCH_GOD, old_god, " "
                                     "당신의 지원금 없이는 빚을 갚을 수가 없었기 때문이다.");
    ...
    case GOD_PAKELLAS:
        simple_god_message("은(는) 당신의 자연적인 마법 재생을 막고 있다.",
                           old_god);
        if (you.duration[DUR_DEVICE_SURGE])
            you.duration[DUR_DEVICE_SURGE] = 0;
        you.exp_docked[old_god] = exp_needed(min<int>(you.max_level, 27) + 1)
    ...
    case GOD_CHEIBRIADOS:
        simple_god_message("은(는) 당신의 움직임을 느리게 하고있다.", old_god);
        break;
...
static void _god_welcome_handle_gear()
{
    ...
    for (int i = EQ_FIRST_EQUIP; i < NUM_EQUIP; i++)
    {
        ...
        {
            mprf(MSGCH_GOD, "%s는 %s를 해제하는 것에 대해 경고했다.",
...
void god_pitch(god_type which_god)
{
    ...
    if (!player_can_join_god(which_god))
    {
        ...
        {
            simple_god_message(" (은)는 당신같은 거지로부터 서비스는 받아들이지 않는다.",
                               which_god);
            if (you.gold == 0)
            {
