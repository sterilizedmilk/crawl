...
static void _eat_chunk(item_def& food)
{
    ...
    case CE_NOXIOUS:
    case CE_NOCORPSE:
        mprf(MSGCH_ERROR, "이 날고기는 (%d) 버그 맛이 난다!", chunk_effect);
        break;
    }
...
bool eat_item(item_def &food)
{
    ...
    mprf("당신은 %s%s 먹었다.", food.name(DESC_PLAIN).c_str(),
                          food.quantity > 1 ? " 중 하나를" : "");
