...
static bool _evoke_horn_of_geryon()
{
    ...
    mprf(MSGCH_SOUND, "당신은 끔찍한 울부짖는 소음을 냈다!");
    ...
    for (int n = 0; n < num; ++n)
    {
        ...
        if (mon && will_anger)
        {
            simple_monster_message(*mon, "는 당신의 신성한 오라가 마음에 안드는 모양이다.");
        }
    }
    if (!created)
        mpr("당신의 부름에 아무런 응답이 없었다.");
    return true;
}
...
static bool _check_crystal_ball()
{
    ...
    {
        mpr("수정구를 작동시키기에는 당신의 남은 마나량이 너무 적다.");
        return false;
    }
...
void zap_wand(int slot)
{
    ...
    {
        mpr("당신은 마법적인 도구를 발동시킬 수 없다.");
        return;
    }

    if (player_under_penance(GOD_PAKELLAS))
    {
        simple_god_message("의 분노가 당신이 도구를 발동시키는 것을 막았다!",
                           GOD_PAKELLAS);
        return;
    }
    ...
    {
        item_slot = prompt_invent_item("무엇을 휘두를 것인가?",
                                       MT_INVLIST,
                                       OBJ_WANDS,
                                       OPER_ZAP);
    }
...
static bool _make_zig(item_def &zig)
{
    ...
    mpr("당신은 모형을 내려놓았다, 지구라트로 향하는 신비로운 포탈이 만들어졌다.");
    return true;
}
...
static bool _lamp_of_fire()
{
    ...
    args.restricts = DIR_TARGET;
    args.mode = TARG_HOSTILE;
    args.top_prompt = "램프를 어디로 조준하겠는가?";
    args.self = CONFIRM_CANCEL;
...
static spret _phantom_mirror()
{
    ...
    args.self = CONFIRM_CANCEL;
    args.top_prompt = "조준: <white>환영 거울</white>";
    args.hitfunc = &tgt;
    ...
    if (!victim || !you.can_see(*victim))
    {
        if (beam.target == you.pos())
            mpr("거울을 자기자신에게 쓰는건 불가능하다.");
        else
            mpr("그곳엔 복제할 만한 것이 아무것도 보이지 않는다.");
        return spret::abort;
    }
    ...
    if (player_will_anger_monster(*victim))
    {
        if (you.get_mutation_level(MUT_NO_LOVE))
            mpr("거울상은 오직 당신을 향한 증오만을 느낄것이다!");
        else
            simple_god_message("는 당신이 이 몬스터를 비추는 것을 금한다.");
        return spret::abort;
    }
    ...
    mons_add_blame(mon, "플레이어에 의해 거울에 비춰짐");
...
bool evoke_item(int slot, bool check_range)
{
    ...
    if (slot == -1)
    {
        slot = prompt_invent_item("어느 아이템을 발동시킬 것인가? (* - 아이템 목록)",
                                   MT_INVLIST,
