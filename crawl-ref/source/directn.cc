...
void full_describe_view()
{
                            ...
                            you.xray_vision ? LOS_NONE : LOS_DEFAULT); ri; ++ri)
    {
        ...
        if (items.empty())
        {
            mprf(MSGCH_ERROR, "더미에서 집어올 물건은 없으나, 가장 위의 물건이 %s이다",
...
bool direction_chooser::handle_signals()
{
    ...
    if (crawl_state.seen_hups)
    {
        ...
        mprf(MSGCH_ERROR, "HUP 신호에 의해 목표 설정이 취소되었다.");
        return true;
    }
    return false;
}
                                              ...
                                              bool* loop_done)
{
    ...
    case CMD_TARGET_WIZARD_DEBUG_PORTAL:
        mprf(MSGCH_DIAGNOSTICS, "%d/%d에 있는 포탈 디버그 중...",
            target().x, target().y);

        marker_result =
            env.markers.property_at(target(), MAT_ANY, "portal_debug");

        mprf(MSGCH_DIAGNOSTICS, "결과: %s!",
            marker_result.empty() ? "없음" : marker_result.c_str());
