...
static void _wanderer_startup_message()
{
    ...
    if (skill_levels <= 2)
    {
        ...
        mpr("당신은 혼란스러운 상태에서 막 깨어났기에, 기억나는 것이 별로 없을 것이다.");
    }
}
...
static void _start_running(int dir, int mode)
{
    ...
    {
        if (dir == RDIR_REST)
            mprf(MSGCH_WARN, "당신은 산성 점액의 벽으로 반복 이동하려고 한다!");
        else
                             ...
                             bool known_shaft)
{
    ...
    {
        mprf("%s 상태에서는 그럴 수 없다.", held_status());
        return false;
    }
        ...
        if (!is_existing_level(level_id(it->id, 1)))
        {
            ...
            if (runes_in_pack() < min_runes)
            {
                if (min_runes == 1)
                    ...
                    mprf("이 곳에 들어가려면 최소한 %d개의 룬이 필요하다.",
                         min_runes);
                return false;
            }
        }
...
static void _experience_check()
{
    ...
    {
        mprf("아쉽지만, %d레벨이 당신이 올릴 수 있는 최대 레벨이다.", you.get_max_xl());
...
static void _do_cycle_quiver(int dir)
{
    ...
    if (next != -1)
    {
        ...
        if (next == cur)
            mpr("다른 투사체가 존재하지 않는다. 다른 물건을 던지기 위해서는 F를 누르시오.");
    }
    else if (cur == -1)
...
static void _do_list_gold()
{
    if (shopping_list.empty())
        mprf("당신은 %d개의 금화를 가지고 있다.", you.gold, you.gold != 1 ? "s" : "");
    else
...
void process_command(command_type cmd)
{
    ...
    case CMD_TOGGLE_AUTOPICKUP:
        ...
        mprf("자동 줍기를 %s.", Options.autopickup_on > 0 ? "on" : "off");
        break;
    ...
    case CMD_CHARACTER_DUMP:
        if (!dump_char(you.your_name))
            mpr("미안하지만, 캐릭터 덤프 파일을 만드는 데 실패했다.");
#ifdef USE_TILE_WEB
        else
    ...
    default:
        ...
        if (crawl_state.game_is_hints())
            ...
            mprf(MSGCH_EXAMINE_FILTER, "그런 명령어는 존재하지 않는다.");
...
static void _prep_input()
{
    ...
    if (you.seen_portals)
    {
        ...
        if (you.seen_portals == 1)
            mprf(MSGCH_GOD, "관문이 어디 위치하는지, 보이기 시작했다.");
        else
            mprf(MSGCH_GOD, "관문들이 어디 위치하는지, 각각 보이기 시작했다.");
...
static void _check_banished()
{
    if (you.banished)
    {
        ...
        if (!player_in_branch(BRANCH_ABYSS))
            mprf(MSGCH_BANISHMENT, "어비스로 빠져들었다! (개꿀!)");
        else if (you.depth < brdepth[BRANCH_ABYSS])
            mprf(MSGCH_BANISHMENT, "어비스의 더 깊은 곳으로 빠져들었다! (야호!)");
        else
            mprf(MSGCH_BANISHMENT, "주변 어비스 공간이 구부러졌다!");
        ...
        if (trap && trap->type == TRAP_GOLUBRIA)
        {
            if (--trap->ammo_qty <= 0)
            {
                if (you.see_cell(c))
                    ...
                    mprf(MSGCH_SOUND, "어디선가 딱딱거리는 소리가 들렸다.");
                trap->destroy();
...
static void _swing_at_target(coord_def move)
{
    ...
    if (you.confused())
    {
        if (!you.is_stationary())
        {
            mpr("당신은 너무 혼란해진 나머지 어딘가를 돌아다니지 않고서는 공격할 수가 없다!");
            return;
        }
...
static void _close_door(coord_def move)
{
    if (you.attribute[ATTR_HELD])
    {
        mprf("%s 상태에서는 문을 닫을 수 없다.", held_status());
        return;
    }
...
static void _do_berserk_no_combat_penalty()
{
    if (you.berserk())
    {
        ...
        {
        case 2:
            mprf(MSGCH_DURATION, "무엇인가를 공격하려는 강렬한 충동이 느껴진다.");
            break;
        case 4:
            mprf(MSGCH_DURATION, "분노가 사그라지는 것 같다.");
            break;
        case 6:
            mprf(MSGCH_DURATION, "분노가 급속히 사라져간다.");
            break;
        }
...
static void _entered_malign_portal(actor* act)
{
    ...
    if (you.can_see(*act))
    {
        mprf("%s %s(이)가 격렬하게 뒤틀리며 차원문에서 튀어나왔다!",
...
static void _move_player(coord_def move)
{
    ...
    if (fedhas_passthrough(targ_monst) && !you.is_stationary())
    {
        ...
        if (!current || !fedhas_passthrough(current))
        {
            ...
            mprf("당신은 %s 사이로 조심스레 %s했다.", walkverb.c_str(),
        ...
        && moving && !beholder && !fmonger)
    {
        ...
        if (you.digging)
        {
            mprf("당신은 %s 사이로 굴착했다.", feature_description_at(targ, false,
...
static void _do_cmd_repeat()
{
    ...
    if (ch == ' ' || ch == CK_ENTER)
    {
        mprf(MSGCH_PROMPT, "반복할 명령키를 입력하시오: ");
