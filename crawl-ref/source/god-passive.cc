...
void qazlal_element_adapt(beam_type flavour, int strength)
{
    ...
    mprf(MSGCH_GOD, "당신은 %s에 대해 %s보호받는 느낌이 들었다.",
         descript.c_str(), you.duration[dur] > 0 ? "더 " : "");
...
static void _wu_jian_lunge(const coord_def& old_pos)
{
    ...
    {
        mprf("당신은 %s을(를) 향해 돌진했지만, 공격 속도가 너무 느려 한 번도 찌르지 못했다.",
...
static void _wu_jian_whirlwind(const coord_def& old_pos)
{
    ...
    for (auto mons : common_targets)
    {
        ...
        if (number_of_attacks == 0)
        {
            mprf("당신은 %s을(를) 공격하기 위해 몸을 회전했지만, 공격 속도가 너무 느려 "
                 "한 번도 베지 못했다.", mons->name(DESC_PLAIN).c_str());
            continue;
        }
        else
        {
            mprf("당신은 돌면서 %s%s을(를) %s베었다.",
...
void wu_jian_wall_jump_effects(const coord_def& old_pos)
{
    ...
    for (auto target : targets)
    {
        ...
        if (number_of_attacks == 0)
        {
            mprf("당신은 %s을(를) 뛰어넘으며 공격했지만, 공격 속도가 너무 느려 착지할 때까지 "
                 "한 번도 공격하지 못했다.", target->name(DESC_PLAIN).c_str());
            continue;
        }
        else
        {
            mprf("당신은 %s을(를) 뛰어넘으며 %s%s공격했다.",
