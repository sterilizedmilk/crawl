...
static void _give_ammo(weapon_type weapon, int plus)
{
    ...
    case WPN_THROWN:
        if (species_can_throw_large_rocks(you.species))
            newgame_make_item(OBJ_MISSILES, MI_LARGE_ROCK, (4 + plus) * 25);
        else if (you.body_size(PSIZE_TORSO) <= SIZE_SMALL)
            newgame_make_item(OBJ_MISSILES, MI_TOMAHAWK, (8 + 2 * plus) * 20);
        else
            newgame_make_item(OBJ_MISSILES, MI_JAVELIN, (5 + plus) * 20);
