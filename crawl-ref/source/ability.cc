                                    ...
                                    bool quiet = false)
{
    ...
    case ABIL_HEPLIAKLQANA_TRANSFERENCE:
        if (hepliaklqana_ancestor() == MID_NOBODY)
        {
            if (!quiet)
            {
                mprf("%s 아직 기억 속에 갇혀 있다!",
                     josa(hepliaklqana_ally_name(), "은").c_str());
            }
            return false;
        }
        return true;
...
static spret_type _do_ability(const ability_def& abil, bool fail)
{
    ...
    case ABIL_TSO_BLESS_WEAPON:
        fail_check();
        simple_god_message("는 당신의 무기 중 하나에, 축복을 내릴 것이다.");
    ...
    case ABIL_PAKELLAS_SUPERCHARGE:
    {
        ...
        string prompt = "정말로 " + wand.name("을", DESC_YOUR)
                           + " 과충전하길 원하는가?";
        ...
        mprf(MSGCH_GOD, "당신의 %s 밝게 빛난다!",
             wand.name("가", DESC_QUALNAME).c_str());
